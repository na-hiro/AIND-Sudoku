...
def naked_twins(values):
    ...
    # Search for naked twin values with two identical numbers from the unit.
    for unit in unitlist:
        # set unit vales
        unit_values = [values[box] for box in unit]
        # search naked twin values from the current unit
        naked_values = [n for n in unit_values if unit_values.count(n) == 2 and len(n) == 2]

        # When naked twins are acquired, corresponding numbers are deleted from all twin companion boxes.
        # Eliminate the value of naked twins from peers
        for peer in naked_values:
#            print(naked_values)
            for val in peer:
#                print(peer, " ", val)
                for box in unit:
